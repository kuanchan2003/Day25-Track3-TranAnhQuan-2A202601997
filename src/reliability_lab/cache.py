...
class ResponseCache:
    ...
    Uses n-gram similarity and false-hit guardrails. The module-level
    _is_uncacheable() and _looks_like_false_hit() helpers protect the
    ...
    def similarity(a: str, b: str) -> float:
        ...
        magnitude_a = math.sqrt(sum(count**2 for count in vector_a.values()))
        magnitude_b = math.sqrt(sum(count**2 for count in vector_b.values()))
...
class SharedRedisCache:
    ...
    Redis commands share cache state across multiple gateway instances.
    ...
    def ping(self) -> bool:
        ...
        except Exception:  # noqa: BLE001 - connectivity probes must degrade to False
            return False
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        for key in self._redis.scan_iter(f"{self.prefix}*"):
            ...
            if score > best_score:
                ...
                best_response = None if cached_response is None else str(cached_response)
                best_score = score
